      ...
      {Array.from(Array(numberOfDependencyBoxes)).map((_, i) => (
        <div key={i} className="container-DependencyBox-input-box">
          ...
          <input
            ...
            onInput={(e) => handleInputChange(e, setSubDependencyListInput, i)}
          />
        </div>
      ))}
      <button onClick={removeDependencyBox}>
        <DoNotDisturbOnIcon sx={{ color: "#780400" }} />
      </button>
      <button onClick={addDependencyBox}>
        <AddCircleIcon sx={{ color: "#1B2B8E" }} />
      </button>
